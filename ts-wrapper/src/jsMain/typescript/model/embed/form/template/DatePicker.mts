...
export class DatePicker {
	...
	readonly $ktClass: 'com.icure.cardinal.sdk.model.embed.form.template.DatePicker' = 'com.icure.cardinal.sdk.model.embed.form.template.DatePicker';

	constructor(partial: Partial<DatePicker> & Pick<DatePicker, "field">) {
	...
	toJSON(): any {
		...
		res['$ktClass'] = 'com.icure.cardinal.sdk.model.embed.form.template.DatePicker'
		return res
	}

	static fromJSON(json: any, ignoreUnknownKeys: boolean = false,
			path: Array<string> = ['DatePicker']): DatePicker {
		if (typeof json != 'object') throw new Error(`Expected json object at path ${path.join("")}`)
		const jCpy = { ...json }
		if (extractEntry(jCpy, '\$ktClass', true, path) !== 'com.icure.cardinal.sdk.model.embed.form.template.DatePicker') throw new Error(`Unexpected value for ${path.join("")} class marker, should be "com.icure.cardinal.sdk.model.embed.form.template.DatePicker". The provided json doesn't represent a DatePicker`)
		const res = new DatePicker({
		...
		if (!ignoreUnknownKeys) {
			...
			if (unused.length > 0) throw new Error(`Unexpected key(s) for json object DatePicker at path ${path.join("")}: ${unused}`)}
		return res
	}
