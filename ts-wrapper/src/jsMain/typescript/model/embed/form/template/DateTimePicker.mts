...
export class DateTimePicker {
	...
	toJSON(): any {
		...
		res['$ktClass'] = 'com.icure.cardinal.sdk.model.embed.form.template.DateTimePicker'
		return res
	}
	...
	static fromJSON(json: any, ignoreUnknownKeys: boolean = false,
		...
		if (!ignoreUnknownKeys) {
			...
			if (unused.length > 0) throw new Error(`Unexpected key(s) for json object DateTimePicker at path ${path.join("")}: ${unused}`)}
		return res
	}
