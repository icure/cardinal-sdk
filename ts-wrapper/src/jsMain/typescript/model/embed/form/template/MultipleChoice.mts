...
export class MultipleChoice {
	...
	toJSON(): object {
		...
		res['$ktClass'] = 'com.icure.cardinal.sdk.model.embed.form.template.MultipleChoice'
		return res
	}
	...
	static fromJSON(json: any, ignoreUnknownKeys: boolean = false,
		...
		if (!ignoreUnknownKeys) {
			...
			if (unused.length > 0) throw new Error(`Unexpected key(s) for json object MultipleChoice at path ${path.join("")}: ${unused}`)}
		return res
	}
