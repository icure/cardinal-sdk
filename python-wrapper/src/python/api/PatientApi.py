# auto-generated file
import json
from cardinal_sdk.model import EncryptedPatient, DecryptedPatient, deserialize_patient, Patient, serialize_patient, EntityReferenceInGroup, User, AccessLevel, PatientDelegateOptions, ShareAllPatientDataOptionsTag, ShareAllPatientDataOptionsResult, StoredDocumentIdentifier, EntityAccessInformation, PatientShareOptions, SubscriptionEventType, EntitySubscriptionConfiguration, GroupScoped
...
class PatientApi:
	...
	async def with_encryption_metadata_and_delegates_async(self, base: Optional[DecryptedPatient], delegates: dict[str, PatientDelegateOptions], user: Optional[User] = None, alternate_root_delegate_id: Optional[str] = None) -> DecryptedPatient:
		def do_decode(raw_result):
			return DecryptedPatient._deserialize(raw_result)
		payload = {
			"base": base.__serialize__() if base is not None else None,
		...
		return await execute_async_method_job(
			...
			True,
			do_decode,
			symbols.kotlin.root.com.icure.cardinal.sdk.py.api.PatientApi.withEncryptionMetadataAndDelegatesAsync,
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)

	def with_encryption_metadata_and_delegates_blocking(self, base: Optional[DecryptedPatient], delegates: dict[str, PatientDelegateOptions], user: Optional[User] = None, alternate_root_delegate_id: Optional[str] = None) -> DecryptedPatient:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"delegates": {k0: v0.__serialize__() for k0, v0 in delegates.items()},
			"user": user.__serialize__() if user is not None else None,
		...
		call_result = symbols.kotlin.root.com.icure.cardinal.sdk.py.api.PatientApi.withEncryptionMetadataAndDelegatesBlocking(
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
...
class PatientApiInGroup:
	...
	async def with_encryption_metadata_and_delegates_async(self, entity_group_id: str, base: Optional[DecryptedPatient], delegates: dict[EntityReferenceInGroup, PatientDelegateOptions], user: Optional[User] = None, alternate_root_delegate_reference: Optional[EntityReferenceInGroup] = None) -> GroupScoped[DecryptedPatient]:
		def do_decode(raw_result):
			return GroupScoped._deserialize(raw_result, lambda x1: DecryptedPatient._deserialize(x1))
		payload = {
			"entityGroupId": entity_group_id,
		...
		return await execute_async_method_job(
			...
			True,
			do_decode,
			symbols.kotlin.root.com.icure.cardinal.sdk.py.api.PatientApi.inGroup.withEncryptionMetadataAndDelegatesAsync,
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)

	def with_encryption_metadata_and_delegates_blocking(self, entity_group_id: str, base: Optional[DecryptedPatient], delegates: dict[EntityReferenceInGroup, PatientDelegateOptions], user: Optional[User] = None, alternate_root_delegate_reference: Optional[EntityReferenceInGroup] = None) -> GroupScoped[DecryptedPatient]:
		payload = {
			"entityGroupId": entity_group_id,
			"base": base.__serialize__() if base is not None else None,
			"delegates": [{ "k": k0.__serialize__(), "v": v0.__serialize__() } for k0, v0 in delegates.items()],
		...
		call_result = symbols.kotlin.root.com.icure.cardinal.sdk.py.api.PatientApi.inGroup.withEncryptionMetadataAndDelegatesBlocking(
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
