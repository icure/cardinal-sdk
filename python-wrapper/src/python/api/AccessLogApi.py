...
class AccessLogApi:
	...
	async def with_encryption_metadata_and_delegates_async(self, base: Optional[DecryptedAccessLog], delegates: dict[str, AccessLogDelegateOptions], patient: Optional[Patient] = None, user: Optional[User] = None, secret_id: SecretIdUseOption = SecretIdUseOptionUseAnySharedWithParent(), alternate_root_delegate_id: Optional[str] = None) -> DecryptedAccessLog:
		def do_decode(raw_result):
			return DecryptedAccessLog._deserialize(raw_result)
		payload = {
			"base": base.__serialize__() if base is not None else None,
	...
	def with_encryption_metadata_and_delegates_blocking(self, base: Optional[DecryptedAccessLog], delegates: dict[str, AccessLogDelegateOptions], patient: Optional[Patient] = None, user: Optional[User] = None, secret_id: SecretIdUseOption = SecretIdUseOptionUseAnySharedWithParent(), alternate_root_delegate_id: Optional[str] = None) -> DecryptedAccessLog:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"delegates": {k0: v0.__serialize__() for k0, v0 in delegates.items()},
			"patient": serialize_patient(patient) if patient is not None else None,
