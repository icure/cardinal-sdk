...
from cardinal_sdk.model import DecryptedInvoice, Patient, User, AccessLevel, SecretIdUseOption, SecretIdUseOptionUseAnySharedWithParent, serialize_patient, serialize_secret_id_use_option, InvoiceDelegateOptions, Invoice, serialize_invoice, EntityReferenceInGroup, EncryptedInvoice, deserialize_invoice, StoredDocumentIdentifier, LabelledOccurence, InvoiceShareOptions, EncryptedInvoicingCode, MediumType, InvoiceType, GroupScoped
...
class InvoiceApi:
	...
	async def with_encryption_metadata_and_delegates_async(self, base: Optional[DecryptedInvoice], patient: Optional[Patient], delegates: dict[str, InvoiceDelegateOptions], user: Optional[User] = None, secret_id: SecretIdUseOption = SecretIdUseOptionUseAnySharedWithParent(), alternate_root_delegate_id: Optional[str] = None) -> DecryptedInvoice:
		def do_decode(raw_result):
			return DecryptedInvoice._deserialize(raw_result)
		payload = {
			"base": base.__serialize__() if base is not None else None,
		...
		return await execute_async_method_job(
			...
			True,
			do_decode,
			symbols.kotlin.root.com.icure.cardinal.sdk.py.api.InvoiceApi.withEncryptionMetadataAndDelegatesAsync,
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)

	def with_encryption_metadata_and_delegates_blocking(self, base: Optional[DecryptedInvoice], patient: Optional[Patient], delegates: dict[str, InvoiceDelegateOptions], user: Optional[User] = None, secret_id: SecretIdUseOption = SecretIdUseOptionUseAnySharedWithParent(), alternate_root_delegate_id: Optional[str] = None) -> DecryptedInvoice:
		payload = {
			"base": base.__serialize__() if base is not None else None,
			"patient": serialize_patient(patient) if patient is not None else None,
			"delegates": {k0: v0.__serialize__() for k0, v0 in delegates.items()},
		...
		call_result = symbols.kotlin.root.com.icure.cardinal.sdk.py.api.InvoiceApi.withEncryptionMetadataAndDelegatesBlocking(
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
...
class InvoiceApiInGroup:
	...
	async def with_encryption_metadata_and_delegates_async(self, entity_group_id: str, base: Optional[DecryptedInvoice], patient: Optional[GroupScoped[Patient]], delegates: dict[EntityReferenceInGroup, InvoiceDelegateOptions], user: Optional[User] = None, secret_id: SecretIdUseOption = SecretIdUseOptionUseAnySharedWithParent(), alternate_root_delegate_reference: Optional[EntityReferenceInGroup] = None) -> GroupScoped[DecryptedInvoice]:
		def do_decode(raw_result):
			return GroupScoped._deserialize(raw_result, lambda x1: DecryptedInvoice._deserialize(x1))
		payload = {
			"entityGroupId": entity_group_id,
		...
		return await execute_async_method_job(
			...
			True,
			do_decode,
			symbols.kotlin.root.com.icure.cardinal.sdk.py.api.InvoiceApi.inGroup.withEncryptionMetadataAndDelegatesAsync,
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)

	def with_encryption_metadata_and_delegates_blocking(self, entity_group_id: str, base: Optional[DecryptedInvoice], patient: Optional[GroupScoped[Patient]], delegates: dict[EntityReferenceInGroup, InvoiceDelegateOptions], user: Optional[User] = None, secret_id: SecretIdUseOption = SecretIdUseOptionUseAnySharedWithParent(), alternate_root_delegate_reference: Optional[EntityReferenceInGroup] = None) -> GroupScoped[DecryptedInvoice]:
		payload = {
			"entityGroupId": entity_group_id,
			"base": base.__serialize__() if base is not None else None,
			"patient": patient.__serialize__(lambda x0: serialize_patient(x0)) if patient is not None else None,
		...
		call_result = symbols.kotlin.root.com.icure.cardinal.sdk.py.api.InvoiceApi.inGroup.withEncryptionMetadataAndDelegatesBlocking(
			self.cardinal_sdk._native,
			json.dumps(payload).encode('utf-8'),
		)
		result_info = create_result_from_json(cast(call_result, c_char_p).value.decode('utf-8'))
