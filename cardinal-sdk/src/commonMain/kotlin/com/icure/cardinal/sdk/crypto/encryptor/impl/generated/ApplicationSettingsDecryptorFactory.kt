...
@InternalIcureApi
internal object ApplicationSettingsDecryptorFactory : EntityDecryptorFactory<EncryptedApplicationSettings, DecryptedApplicationSettings> {
	override fun create(
	...
	): EntityDecryptor<EncryptedApplicationSettings, DecryptedApplicationSettings> {
		...
		require(manifest?.extensionsManifestsByModelVersion.isNullOrEmpty()) {
			"ApplicationSettings is not Extendable and does not support extensions decryption, but its manifest defines extensionsManifestsByModelVersion."
		}
		val patchDecryptedSelfJson = entityManifestName?.let { encryptorsFactoryContext.getManifestDecryptedJsonPatcher(it) }
		return ApplicationSettingsDecryptor(
...
@InternalIcureApi
private class ApplicationSettingsDecryptor(
	...
	override suspend fun decrypt(
		...
		val result =
			DecryptedApplicationSettings(
				...
				settings =
					encryptedContentDecoder.decodeDecrypted(
						decryptedContent["settings"]?.also { usedEncryptedContent += "settings" },
						encryptedEntity.settings,
						entityCustomisedModelVersion,
					),
				encryptedSettings =
					encryptedContentDecoder.decodeDecrypted(
						decryptedContent["encryptedSettings"]?.also { usedEncryptedContent += "encryptedSettings" },
