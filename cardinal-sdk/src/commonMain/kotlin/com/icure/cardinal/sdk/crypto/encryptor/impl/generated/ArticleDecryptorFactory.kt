...
@InternalIcureApi
internal object ArticleDecryptorFactory : EntityDecryptorFactory<EncryptedArticle, DecryptedArticle> {
	override fun create(
	...
	): EntityDecryptor<EncryptedArticle, DecryptedArticle> {
		...
		require(manifest?.extensionsManifestsByModelVersion.isNullOrEmpty()) {
			"Article is not Extendable and does not support extensions decryption, but its manifest defines extensionsManifestsByModelVersion."
		}
		val patchDecryptedSelfJson = entityManifestName?.let { encryptorsFactoryContext.getManifestDecryptedJsonPatcher(it) }
		return ArticleDecryptor(
...
@InternalIcureApi
private class ArticleDecryptor(
	private val encryptedContentDecoder: Json,
	patchDecryptedSelfJson: ((JsonObject) -> JsonObject)?,
	cryptoService: CryptoService,
	unversionedEntitiesDecryptedJsonStrictness: DecryptedJsonStrictness,
) : AbstractEntityDecryptor<EncryptedArticle, DecryptedArticle>(
	...
	override suspend fun decrypt(
		...
		encryptedEntity: EncryptedArticle,
		...
		val result =
			DecryptedArticle(
				...
				classification =
					encryptedContentDecoder.decodeDecrypted(
						decryptedContent["classification"]?.also { usedEncryptedContent += "classification" },
						encryptedEntity.classification,
						entityCustomisedModelVersion,
					),
		...
		if (entityCustomisedModelVersion == null &&
			...
			throw UnexpectedEncryptedContentException(
				"The Article encrypted content contains unexpected fields: ${decryptedContent.keys - usedEncryptedContent}",
			)
		}
		return result
	}
