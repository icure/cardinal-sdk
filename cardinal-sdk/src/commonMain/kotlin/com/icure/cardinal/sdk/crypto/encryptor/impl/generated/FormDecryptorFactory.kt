...
@InternalIcureApi
internal object FormDecryptorFactory : EntityDecryptorFactory<EncryptedForm, DecryptedForm> {
	override fun create(
	...
	): EntityDecryptor<EncryptedForm, DecryptedForm> {
		...
		require(manifest?.extensionsManifestsByModelVersion.isNullOrEmpty()) {
			"Form is not Extendable and does not support extensions decryption, but its manifest defines extensionsManifestsByModelVersion."
		}
		val patchDecryptedSelfJson = entityManifestName?.let { encryptorsFactoryContext.getManifestDecryptedJsonPatcher(it) }
		return FormDecryptor(
...
@InternalIcureApi
private class FormDecryptor(
	private val encryptedContentDecoder: Json,
	patchDecryptedSelfJson: ((JsonObject) -> JsonObject)?,
	cryptoService: CryptoService,
	unversionedEntitiesDecryptedJsonStrictness: DecryptedJsonStrictness,
) : AbstractEntityDecryptor<EncryptedForm, DecryptedForm>(
	...
	override suspend fun decrypt(
		...
		if (entityCustomisedModelVersion == null &&
			...
			throw UnexpectedEncryptedContentException(
				"The Form encrypted content contains unexpected fields: ${decryptedContent.keys - usedEncryptedContent}",
			)
		}
		return result
	}
