...
class RawHealthcarePartyApiImpl(
	...
	override suspend fun getConflictingEntitiesIds(): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "hcparty", "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()

	override suspend fun getConflictsForEntity(entityId: String): HttpResponse<List<HealthcareParty>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
	...
	): HttpResponse<ConflictResolutionResult<HealthcareParty>> =
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "hcparty", "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun autoSolveConflicts(entityIds: List<String>): HttpResponse<List<MergeResult>> =
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "hcparty", "conflicts", "solve")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(entityIds)
	...
	override suspend fun getConflictingEntitiesIdsInGroup(groupId: String): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "hcparty", "inGroup", groupId, "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()
	...
	override suspend fun declareConflictWinnerInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "hcparty", "inGroup", groupId, "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun autoSolveConflictsInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "hcparty", "inGroup", groupId, "conflicts", "solve")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(entityIds)
