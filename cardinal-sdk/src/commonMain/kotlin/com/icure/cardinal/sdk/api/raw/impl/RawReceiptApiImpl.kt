...
class RawReceiptApiImpl(
	...
	override suspend fun getConflictingEntitiesIds(): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()

	override suspend fun getConflictsForEntity(entityId: String): HttpResponse<List<EncryptedReceipt>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "conflicts", "of")
	...
	override suspend fun declareConflictWinner(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun matchReceiptsByCustomFilter(filter: CustomFilter): HttpResponse<PaginatedList<IdWithValue>> =
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "matchByCustom")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(filter)
	...
	override suspend fun getConflictingEntitiesIdsInGroup(groupId: String): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "inGroup", groupId, "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()
	...
	override suspend fun getConflictsForEntityInGroup(
		...
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "inGroup", groupId, "conflicts", "of")
				parameter("entityId", entityId)
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
	...
	override suspend fun declareConflictWinnerInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "inGroup", groupId, "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun matchReceiptsByCustomFilterInGroup(
		groupId: String,
		filter: CustomFilter,
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "receipt", "inGroup", groupId, "matchByCustom")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(filter)
