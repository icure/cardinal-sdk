...
class RawFormApiImpl(
	...
	override suspend fun getConflictingEntitiesIds(): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "form", "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()

	override suspend fun getConflictsForEntity(entityId: String): HttpResponse<List<EncryptedForm>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
	...
	override suspend fun declareConflictWinner(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "form", "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun autoSolveConflicts(entityIds: List<String>): HttpResponse<List<MergeResult>> =
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "form", "conflicts", "solve")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(entityIds)
	...
	override suspend fun getConflictingEntitiesIdsInGroup(groupId: String): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "form", "inGroup", groupId, "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()
	...
	override suspend fun declareConflictWinnerInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "form", "inGroup", groupId, "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun autoSolveConflictsInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "form", "inGroup", groupId, "conflicts", "solve")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(entityIds)
