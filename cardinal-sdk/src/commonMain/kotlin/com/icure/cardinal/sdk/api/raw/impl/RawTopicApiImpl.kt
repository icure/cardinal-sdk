...
class RawTopicApiImpl(
	...
	override suspend fun getConflictingEntitiesIds(): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()

	override suspend fun getConflictsForEntity(entityId: String): HttpResponse<List<EncryptedTopic>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
	...
	): HttpResponse<ConflictResolutionResult<EncryptedTopic>> =
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun autoSolveConflicts(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "conflicts", "solve")
	...
	override suspend fun getConflictingEntitiesIdsInGroup(groupId: String): HttpResponse<List<String>> =
		get(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "inGroup", groupId, "conflicts")
				parameter("ts", GMTDate().timestamp)
			}
			accept(Application.Json)
		}.wrap()
	...
	override suspend fun declareConflictWinnerInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "inGroup", groupId, "conflicts", "winner")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(request)
	...
	override suspend fun autoSolveConflictsInGroup(
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "inGroup", groupId, "conflicts", "solve")
				parameter("strategy", strategy?.dtoSerialName)
			}
			contentType(Application.Json)
			accept(Application.Json)
	...
	override suspend fun matchTopicsByCustomFilter(filter: CustomFilter): HttpResponse<PaginatedList<IdWithValue>> =
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "matchByCustom")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(filter)
	...
	override suspend fun matchTopicsByCustomFilterInGroup(
		groupId: String,
		filter: CustomFilter,
		...
		post(authProvider) {
			url {
				takeFrom(apiUrl)
				appendPathSegments("rest", "v2", "topic", "inGroup", groupId, "matchByCustom")
			}
			contentType(Application.Json)
			accept(Application.Json)
			setBody(filter)
