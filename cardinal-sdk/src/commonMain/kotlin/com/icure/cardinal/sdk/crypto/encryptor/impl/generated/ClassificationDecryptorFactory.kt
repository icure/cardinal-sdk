...
@InternalIcureApi
internal object ClassificationDecryptorFactory : EntityDecryptorFactory<EncryptedClassification, DecryptedClassification> {
	override fun create(
	...
	): EntityDecryptor<EncryptedClassification, DecryptedClassification> {
		...
		require(manifest?.extensionsManifestsByModelVersion.isNullOrEmpty()) {
			"Classification is not Extendable and does not support extensions decryption, but its manifest defines extensionsManifestsByModelVersion."
		}
		val patchDecryptedSelfJson = entityManifestName?.let { encryptorsFactoryContext.getManifestDecryptedJsonPatcher(it) }
		return ClassificationDecryptor(
...
@InternalIcureApi
private class ClassificationDecryptor(
